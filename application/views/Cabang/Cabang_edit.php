...
<div class="card">
  ...
  <div class="form-group <?=form_error('kode') ? 'has-error' : null?>">
    ...
    <input type="hidden" name="cabang_id" value="<?=$row->cabang_id?>">
    <input type="text" class="form-control" name="kode" onkeyup="this.value = this.value.toUpperCase()"
     value="<?=$this->input->post('kode') ?? $row->kode?>" id="kode" placeholder="Kode Cabang ...">
  ...
  <div class="form-group class="form-group <?=form_error('name') ? 'has-error' : null?>">
    ...
    <input type="text" class="form-control" name="name" value="<?=$this->input->post('name') ?? $row->name?>" id="name" placeholder="Nama Cabang">
